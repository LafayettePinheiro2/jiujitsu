<?php
	...
	<header class="col100 post-header  post-header-loop">
		...
		<?php elseif ( $title_size != 'hide' ) : ?>
			...
			<h2 itemprop="name" title="<?php the_title(); ?>" ><a href="<?php the_permalink(); ?>#post-<?php the_ID(); ?>" itemprop="url"><?php echo TheChameleon_the_title_maxlength( get_the_title(), $title_size ); ?></a></h2>
	...
	<section class="col100 post-content-loop">
		...
		<?php if ( has_post_thumbnail() or TheChameleon_has_post_audio( $post->ID ) ) : ?>	

			<figure class="post-media post-media-standard post-media-loop post-media-video-loop aligncenter">				

					<?php echo TheChameleon_get_post_featured_media( $post->ID, 'audio', array() ); ?>
